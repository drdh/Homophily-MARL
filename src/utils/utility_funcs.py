...
def make_video_from_image_dir(vid_path, img_folder, video_name='trajectory', fps=5):
    ...
    images.sort(key=lambda f: int(f.replace(".png",'')))
...
def return_view(grid, pos, row_size, col_size, pad_rgb=False):
...
def pad_if_needed(left_edge, right_edge, top_edge, bot_edge, matrix, pad_rgb=False):
    ...
    return pad_matrix(left_pad, right_pad, top_pad, bot_pad, matrix, 0, pad_rgb), left_pad, top_pad


def pad_matrix(left_pad, right_pad, top_pad, bot_pad, matrix, const_val=1, pad_rgb=False):
    if pad_rgb:
        pad_mat = np.pad(matrix, ((left_pad, right_pad), (top_pad, bot_pad), (0, 0)),
                         'constant', constant_values=(const_val))
    else:
        pad_mat = np.pad(matrix, ((left_pad, right_pad), (top_pad, bot_pad)),
                         'constant', constant_values=(const_val)) # constant_values=(const_val, const_val)
    return pad_mat
